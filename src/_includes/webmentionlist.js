                  ...
                  `<li class="mt-8">
                    <article class="block h-cite">
                      <div class="flex items-center flex-wrap">
                        <a class="text-black dark:text-white flex items-center flex-wrap border-0 no-underline p-author h-card" href="${reply.url}"><img class="inline h-12 w-12 object-cover mr-2 rounded-full u-photo" src="${reply.author.photo}" alt="${reply.author.name}"><strong class="p-name">${reply.author.name}</strong></a>&nbsp;<span class="text-sm"><time class="italic dt-published" datetime="${reply.published}">${this.readableDateFromISO(reply.published)}</time></span>
                      </div>
              ...
              <ol class="list-none p-0">
                ${mentions.map(mention =>
                  `<li class="mt-8">
                    <article class="block h-cite">
                      <div class="flex items-center flex-wrap">
                        <a class="text-black dark:text-white flex items-center flex-wrap border-0 no-underline p-author h-card" href="${mention.url}"><img class="inline h-12 w-12 object-cover mr-2 rounded-full u-photo" src="${mention.author.photo}" alt="${mention.author.name}"><strong class="p-name">${mention.author.name}</strong></a>&nbsp;<span class="text-sm"><time class="italic dt-published" datetime="${mention.published}">${this.readableDateFromISO(mention.published)}</time></span>
                      </div>
